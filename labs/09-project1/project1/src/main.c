...
ISR(TIMER2_OVF_vect)
{
    ...
    if (joy_sw == 0)
    {
        ...
        {
            // otherwise start the clock
            uart_puts("STOP"); // for debug
            stop_timer();
        }
    }
    joy_sw_state = joy_sw;

    /*uint8_t rot_sw = (PINB & (1<<ROT_SW)); // debug
    if (rot_sw==0)
    {
        uart_puts("STOP");
        stop_timer();
